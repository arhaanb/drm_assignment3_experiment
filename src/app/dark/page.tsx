...
// Direct access to dark pattern prototype (for researcher testing)
// Skips demographics — creates no DB records
export default function DarkPrototypePage() {
  ...
  useEffect(() => {
    if (!assignedGroup) {
      ...
      initExperiment("preview-dark", "preview-dark-session", "dark_pattern");
    }
    setReady(true);
  ...
  return (
    <main className="flex-1 flex items-start justify-center min-h-screen">
      <div className="w-full">
        <div className="bg-yellow-100 text-yellow-800 text-xs text-center py-1.5 font-medium">
          Preview Mode — Dark Pattern Prototype
        </div>
